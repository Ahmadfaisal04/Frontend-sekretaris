...
export default function SuratKeluar() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      data.append('ditujukan', formData.ditujukan);
  ...
  const handleDelete = async (id) => {
    ...
    if (result.isConfirmed) {
      setLoading(true);
      try {
        const response = await fetch(API_ENDPOINTS.SURAT_KELUAR_DELETE(id), {
          method: 'DELETE',
          headers: getHeaders(),
        });
        if (!response.ok) throw new Error('Gagal menghapus data');
        Swal.fire('Berhasil!', 'Data surat keluar telah dihapus.', 'success');
        fetchData();
      } catch (err) {
